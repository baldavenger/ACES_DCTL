...
{
float3 out;
out.x = _pow10f(a.x); out.y = _pow10f(a.y); out.z = _pow10f(a.z);
return out;
}
...
linear = log3g10 < 0.0f ? (log3g10 / g) : (_powf(10.0f, log3g10 / a) - 1.0f) / b;
linear = linear - c;
return linear;
}
...
{segmented_spline_c5_fwd( 0.18f * _powf(2.0f, -12.0f) ), 0.0001f}, {segmented_spline_c5_fwd( 0.18f ), 10.0f},
{segmented_spline_c5_fwd( 0.18 * _powf(2.0f, 10.0f) ), 1000.0f}, 3.0f, 0.06f};
return A;
};
...
{segmented_spline_c5_fwd( 0.18f * _powf(2.0f, -12.0f) ), 0.0001f}, {segmented_spline_c5_fwd( 0.18f ), 10.0f},
{segmented_spline_c5_fwd( 0.18f * _powf(2.0f, 11.0f) ), 2000.0f}, 3.0f, 0.12f};
return A;
};
...
{segmented_spline_c5_fwd( 0.18f * _powf(2.0f, -12.0f) ), 0.0001f}, {segmented_spline_c5_fwd( 0.18f ), 10.0f},
{segmented_spline_c5_fwd( 0.18f * _powf(2.0f, 12.0f) ), 4000.0f}, 3.0f, 0.3f};
return A;
};
...
{
float3 ACESproxy;
ACESproxy.x = In.x * 4095.0f;
ACESproxy.y = In.y * 4095.0f;
ACESproxy.z = In.z * 4095.0f;
float3 lin_AP1;
...
{
if (x > 0.149659f)
return (_powf(10.0f, (x - 0.385537f) / 0.247189f) - 0.052272f) / 5.555556f;
else
...
float3 lin;
lin.x = CanonLog_to_linear( (pmtx.x * 876.0f + 64.0f) / 1023.0f );
lin.y = CanonLog_to_linear( (pmtx.y * 876.0f + 64.0f) / 1023.0f );
lin.z = CanonLog_to_linear( (pmtx.z * 876.0f + 64.0f) / 1023.0f );
float3 aces;
aces.x = 0.561538969f * lin.x + 0.402060105f * lin.y + 0.036400926f * lin.z;
aces.y = 0.092739623f * lin.x + 0.924121198f * lin.y - 0.016860821f * lin.z;
aces.z = 0.084812961f * lin.x + 0.006373835f * lin.y + 0.908813204f * lin.z;
...
float3 lin;
lin.x = 0.9f * CanonLog3_to_linear( CLogIRE.x);
lin.y = 0.9f * CanonLog3_to_linear( CLogIRE.y);
lin.z = 0.9f * CanonLog3_to_linear( CLogIRE.z);
...
__DEVICE__ inline float3 IDT_Sony_SLog1_SGamut_12( float3 In)
{
mat3 SGAMUT_TO_ACES_MTX = { { 0.754338638f, 0.021198141f, -0.009756991f }, { 0.133697046f, 1.005410934f, 0.004508563f }, { 0.111968437f, -0.026610548f, 1.005253201f } };
...
__DEVICE__ inline float3 IDT_Sony_SLog2_SGamut_Tungsten_10( float3 In)
{
...
__DEVICE__ inline TsParams init_TsParams( float minLum, float maxLum, float expShift = 0.0f)
{
...
float3 rgbPre = linCV_2_Y_f3( linearCV, 48.0f, _powf(10.0f, _log10f(0.02f)));
float3 rgbPost;
...
__DEVICE__ inline float3 sat_adjust( float3 rgbIn, float SAT_FACTOR, float3 RGB2Y = make_float3(RGBtoXYZ( REC709_PRI).c0.y, RGBtoXYZ( REC709_PRI).c1.y, RGBtoXYZ( REC709_PRI).c2.y))
{
...
ych.y = _sqrtf(yo);
ych.z = _atan2f(yab.z, yab.y) * (180.0f / 3.14159265358979323846264338327950288f);
...
float3 yab;
yab.x = ych.x;
float h = ych.z * (3.14159265358979323846264338327950288f / 180.0f);
yab.y = ych.y * _cosf(h);
yab.z = ych.y * _sinf(h);
return yab;
}
...
float3 linearCV;
linearCV.x = Y_2_linCV( rgbPost.x, 48.0f, _powf(10.0f, _log10f(0.02f)));
linearCV.y = Y_2_linCV( rgbPost.y, 48.0f, _powf(10.0f, _log10f(0.02f)));
linearCV.z = Y_2_linCV( rgbPost.z, 48.0f, _powf(10.0f, _log10f(0.02f)));
...
rgbPost = add_f_f3( -_pow10f(-4.4550166483), rgbPost);
...
float3 rgbPre;
rgbPre.x = linCV_2_Y( linearCV.x, 48.0f, _powf(10.0f, _log10f(0.02f)));
rgbPre.y = linCV_2_Y( linearCV.y, 48.0f, _powf(10.0f, _log10f(0.02f)));
rgbPre.z = linCV_2_Y( linearCV.z, 48.0f, _powf(10.0f, _log10f(0.02f)));
