...
__DEVICE__ float inline segmented_spline_c5_fwd( float x)
{
  ...
  // set to HALF_MIN.
  float logx = _log10f( _fmaxf(x, HALF_MIN )); 
...
{  
  ...
  const float KNOT_INC_LOW = (_log10f(C.midPoint.x) - _log10f(C.minPoint.x)) / (N_KNOTS_LOW - 1.);
  const float KNOT_INC_HIGH = (_log10f(C.maxPoint.x) - _log10f(C.midPoint.x)) / (N_KNOTS_HIGH - 1.);
  ...
  } else { //if ( logy >= _log10f(C.maxPoint.y) ) {
...
__DEVICE__ inline SegmentedSplineParams_c9 ODT_48nits()
{
  ...
  {segmented_spline_c5_fwd( 0.18f * _powf(2.0f, -6.5f) ),  0.02f},
  {segmented_spline_c5_fwd( 0.18f ), 4.8f},
  {segmented_spline_c5_fwd( 0.18f * _powf(2.0f, 6.5f) ), 48.0f},
  0.0f, 0.04f};
  return A;
};
...
__DEVICE__ inline SegmentedSplineParams_c9 ODT_1000nits()
{
  ...
  {segmented_spline_c5_fwd( 0.18f * _powf(2.0f, -12.0f) ), 0.0001f},
  {segmented_spline_c5_fwd( 0.18f ), 10.0f},
  {segmented_spline_c5_fwd( 0.18 * _powf(2.0f, 10.0f) ), 1000.0f},
  3.0f, 0.06f};
  return A;
};
...
__DEVICE__ inline SegmentedSplineParams_c9 ODT_2000nits()
{
  ...
  {segmented_spline_c5_fwd( 0.18f * _powf(2.0f, -12.0f) ), 0.0001f},
  {segmented_spline_c5_fwd( 0.18f ), 10.0f},
  {segmented_spline_c5_fwd( 0.18f * _powf(2.0f, 11.0f) ),  2000.0f},
  3.0f, 0.12f};
  return A;
};
...
__DEVICE__ inline SegmentedSplineParams_c9 ODT_4000nits()
{
  ...
  {segmented_spline_c5_fwd( 0.18f * _powf(2.0f, -12.0f) ), 0.0001f},
  {segmented_spline_c5_fwd( 0.18f ), 10.0f},
  {segmented_spline_c5_fwd( 0.18f * _powf(2.0f, 12.0f) ), 4000.0f},
  3.0f, 0.3f};
  return A;
};
...
{    
  ...
  if ( logx <= _log10f(C.minPoint.x) ) { 
    ...
    float knot_coord = (N_KNOTS_LOW-1) * (logx-log10(C.minPoint.x))/(_log10f(C.midPoint.x)-log10(C.minPoint.x));
    int j = knot_coord;
    float t = knot_coord - j;
